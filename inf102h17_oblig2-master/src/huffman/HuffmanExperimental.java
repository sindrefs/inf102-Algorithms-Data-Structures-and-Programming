...
public class HuffmanExperimental {

	private static final int OCCURRENCES = 50;
	...
	private static void encode(String fileName) throws FileNotFoundException {
		...
		for (Map.Entry<String, String> entry : map.entrySet()) {
			out.print(entry.getKey() + "=" + (char) (Integer.parseInt(entry.getValue(), 2)) + "/");
		}
		...
		while (scanner.hasNextLine()) {
			...
			for (String string : parts) {
				...
				if (binaryRep.containsKey(string)) {
					newStr = Character.toString((char) (Integer.parseInt(binaryRep.get(string), 2)));
				} else
					newStr = string;
	...
	private static void decode(String fileName) throws FileNotFoundException {
		...
		for (int i = 0; i < line1_parts.length - 1; i++) {
